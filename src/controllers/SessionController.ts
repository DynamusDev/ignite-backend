...
const oauth2Client = new OAuth2(
	'110527689549-4ba6n6gok3i10arbjpv4vvekud0620lc.apps.googleusercontent.com',
	'BELbgHcPVGJUvw7UuQmpUMLU',
	'https://developers.google.com/oauthplayground',
);

oauth2Client.setCredentials({
	refresh_token:
		'1//04gBVVyb0sXpOCgYIARAAGAQSNwF-L9IrnipkcKpnv9QEb-gX2INDdPMKdgB0x4unGFBpyhfEr6Xh0Wo7q4XRYhx3up1oKtYl7wQ',
});
...
export default {
  async create(request: Request, response: Response) {
    ...
    const session = getRepository(Student);
    const comp = getRepository(Company)
    const sch = getRepository(School)
  ...
  async forgotPasswordStudent(request: Request, response: Response) {
...
Equipe Ignite`,
  ...
  async forgotPasswordCompany(request: Request, response: Response) {
    const { email } = request.body;
    const session = getRepository(Company);
  ...
  async forgotPasswordSchool(request: Request, response: Response) {
    const { email } = request.body;
    const session = getRepository(School);
