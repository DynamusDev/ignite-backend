"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const typeorm_1 = require("typeorm");
const School_1 = __importDefault(require("../models/School"));
const bcryptjs_1 = __importDefault(require("bcryptjs"));
exports.default = {
    async create(request, response) {
        const { name, email, password } = request.body;
        console.log(request.body);
        const schoolRepository = typeorm_1.getRepository(School_1.default);
        const school = await schoolRepository.findOne({ email: email });
        if (school) {
            return response.status(400).json({ error: 'Essa empresa já está cadastrada no nosso sistema' });
        }
        else {
            let hashedPassword;
            try {
                hashedPassword = await bcryptjs_1.default.hash(password, 12);
            }
            catch (err) {
                console.error(err);
            }
            const newSchool = schoolRepository.create({
                name,
                email,
                password: hashedPassword,
                createdAt: new Date(),
                logo: '',
            });
            await schoolRepository.save(newSchool);
            return response.status(200).json({
                status: 200,
                message: 'a escola ' + name + ' foi cadastrada!!!',
                school: newSchool
            });
        }
    },
    async edit(request, response) {
        const { id } = request.params;
        const { name, phone, email, password, logo, address, cnpj, specialties } = request.body;
        console.log(request.body);
        const schoolRepository = typeorm_1.getRepository(School_1.default);
        let hashedPassword;
        try {
            hashedPassword = await bcryptjs_1.default.hash(password, 12);
        }
        catch (err) {
            console.error(err);
        }
        const school = await schoolRepository.findOne(id, { relations: ['student'] });
        schoolRepository.merge(school, {
            name,
            phone,
            email,
            password: hashedPassword,
            logo,
            address,
            cnpj,
            specialties,
            updatedAt: new Date()
        });
        await schoolRepository.save(school);
        return response.status(200).json({
            status: 200,
            message: 'Os dados da escola ' + name + ' foram atualizados!!!',
            school
        });
    },
    async editWithoutPassword(request, response) {
        const { id } = request.params;
        const { name, phone, email, logo, address, cnpj, specialties } = request.body;
        console.log(request.body);
        const schoolRepository = typeorm_1.getRepository(School_1.default);
        const school = await schoolRepository.findOne(id, { relations: ['student'] });
        schoolRepository.merge(school, {
            name,
            phone,
            email,
            logo,
            address,
            cnpj,
            specialties,
            updatedAt: new Date()
        });
        await schoolRepository.save(school);
        return response.status(200).json({
            status: 200,
            message: 'Os dados da escola ' + name + ' foram atualizados!!!',
            school
        });
    },
    async index(request, response) {
        const schoolRepository = typeorm_1.getRepository(School_1.default);
        const schools = (await schoolRepository.find({ where: { deletedAt: null }, relations: ['student'] }));
        return response.status(200).json({
            status: 200,
            message: 'Succesfuly',
            schools
        });
    },
    async indexAll(request, response) {
        const schoolRepository = typeorm_1.getRepository(School_1.default);
        const schools = (await schoolRepository.find());
        return response.status(200).json({
            status: 200,
            message: 'Succesfuly',
            schools
        });
    },
    async especific(request, response) {
        const schoolRepository = typeorm_1.getRepository(School_1.default);
        const { id } = request.params;
        const school = await schoolRepository.findOneOrFail(id, { relations: ['student'] });
        return response.status(200).json({
            status: 200,
            message: 'Succesfuly',
            school
        });
    },
    async delete(request, response) {
        const schoolRepository = typeorm_1.getRepository(School_1.default);
        const { id } = request.params;
        const school = await schoolRepository.findOneOrFail(id);
        schoolRepository.merge(school, {
            deletedAt: new Date()
        });
        await schoolRepository.save(school);
        return response.status(200).json({
            status: 200,
            message: `Succesfuly school deleted`,
        });
    }
};
